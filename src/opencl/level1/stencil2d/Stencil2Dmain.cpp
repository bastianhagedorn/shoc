...
template <class T>
void DoTest(std::string testName, cl_device_id dev, cl_context ctx, cl_command_queue queue,
	    ResultDatabase &resultDB, OptionParser &opts, std::string compileFlags) {
...
// obtain factories for the stencils we use
		...
		if (cwrank == 0) {
			...
			if (!matrixFilenameBase.empty()) {
				std::cout << "\nReading expected stencil operation result from "
					     "file for later comparison with OpenCL output\n"
					  << std::endl;
			} else {
				std::cout
				    << "\nPerforming stencil operation on host for later "
				       "comparison with OpenCL output\n"
...
#endif // defined(PARALLEL)
		Matrix2D<T> expected(arrayDims[0] + 2 * haloWidth, arrayDims[1] + 2 * haloWidth);
		Initialize<T> init(seed, haloWidth, haloVal);

		bool haveExpectedData = false;
		if (!matrixFilenameBase.empty()) {
			bool readOK =
			    ReadMatrixFromFile(expected, GetMatrixFileName<T>(matrixFilenameBase));
			if (readOK) {
				...
				if ((expected.GetNumRows() != arrayDims[0] + 2 * haloWidth) ||
					...
					std::cerr << "The matrix read from file \'"
						  ...
						  << "\' does not match the matrix size specified "
						     "on the command line.\n";
...
#if defined(PARALLEL)
		MPIOpenCLStencil<T> *mpiTestStencil =
		    static_cast<MPIOpenCLStencil<T> *>(testStencil);
		...
		for (unsigned int pass = 0; pass < nPasses; pass++) {
			...
			resultDB.AddResult(testName, experimentDescriptionStr.str(), "GFLOPS",
					   gflops);
			if (beVerbose) {
...
// validate the result
			...
			validater->ValidateResult(expected, data, valErrThreshold, nValErrsToPrint);
		}
	} catch (...) {
...
#endif // defined(PARALLEL)
		DoTest<double>("DP_Sten2D", dev, ctx, queue, resultDB, op,
...
void addBenchmarkSpecOptions(OptionParser &opts) {
	...
	opts.addOption("saveExpMatrixFile", OPT_STRING, "",
		       "Basename for output file(s) that will hold expected matrices");

	opts.addOption("warmupPasses", OPT_INT, "1",
		       "Number of warmup passes to do before starting timings", 'w');
