...
template <class T>
OpenCLStencil<T>::OpenCLStencil(T wCenter, T wCardinal, T wDiagonal, size_t _lRows, size_t _lCols,
				cl_device_id _dev, cl_context _ctx, cl_command_queue _queue)
    : Stencil<T>(wCenter, wCardinal, wDiagonal), lRows(_lRows), lCols(_lCols), device(_dev),
...
// use RTTI to determine type?
	...
	if (sizeof(T) == sizeof(float)) {
		...
		throw InvalidArgValue(
		    "template type T has unrecognized size (must be float or double)");
	}
#endif // READY

	// associate our OpenCL kernel source with a Program
	cl_int clErr;
	cl_program prog = clCreateProgramWithSource(context, 1, &cl_source_stencil2d, NULL, &clErr);
	CL_CHECK_ERROR(clErr);

	std::ostringstream buildOptions;
	buildOptions << "-DLROWS=" << lRows << " -DLCOLS=" << lCols << " -D" << precision;

	try {
		clErr = clBuildProgram(prog, 1, &device, buildOptions.str().c_str(), NULL, NULL);
		CL_CHECK_ERROR(clErr);
	} catch (...) {
		// the build failed - why?
		std::string msg = "build failed";
		size_t nBytesNeeded = 0;
		clErr = clGetProgramBuildInfo(prog, device, CL_PROGRAM_BUILD_LOG, 0, NULL,
					      &nBytesNeeded);
		if (clErr == CL_SUCCESS) {
			char *buildLog = new char[nBytesNeeded + 1];
			clErr = clGetProgramBuildInfo(prog, device, CL_PROGRAM_BUILD_LOG,
						      nBytesNeeded + 1, buildLog, NULL);
			if (clErr == CL_SUCCESS) {
				msg = buildLog;
			}
			delete[] buildLog;
...
template <class T>
void OpenCLStencil<T>::SetCopyRectKernelArgs(cl_mem dest, int destOffset, int destPitch, cl_mem src,
					     int srcOffset, int srcPitch, int width, int height) {
	int clErr;
...
template <class T>
void OpenCLStencil<T>::SetStencilKernelArgs(cl_mem currData, cl_mem newData, int alignment,
					    T wCenter, T wCardinal, T wDiagonal,
...
template <class T> void OpenCLStencil<T>::ClearWaitEvents(std::vector<cl_event> &waitEvents) {
	for (std::vector<cl_event>::iterator iter = waitEvents.begin(); iter != waitEvents.end();
	     ++iter) {
		clReleaseEvent(*iter);
	}
	waitEvents.clear();
...
template <class T> void OpenCLStencil<T>::operator()(Matrix2D<T> &mtx, unsigned int nIters) {
	...
	cl_int clErr = 0;
	cl_mem dataBuf1 =
	    clCreateBuffer(context, CL_MEM_READ_WRITE, mtx.GetDataSize(), NULL, &clErr);
	CL_CHECK_ERROR(clErr);
	cl_mem dataBuf2 =
	    clCreateBuffer(context, CL_MEM_READ_WRITE, mtx.GetDataSize(), NULL, &clErr);
	CL_CHECK_ERROR(clErr);
	...
	cl_event writeEvt;
	clErr = clEnqueueWriteBuffer(queue, *currData,
				     CL_FALSE,		// blocking
				     0,			// offset
				     mtx.GetDataSize(), // cb
				     mtx.GetFlatData(),
	...
	clErr = clEnqueueCopyBuffer(queue, *currData, *newData, 0, 0, rowExtent, waitEvents.size(),
				    waitEvents.empty() ? NULL : &waitEvents.front(), NULL);
	CL_CHECK_ERROR(clErr);
	ClearWaitEvents(waitEvents);

	clErr = clEnqueueCopyBuffer(queue, *currData, *newData, (mtx.GetNumRows() - 1) * rowExtent,
				    (mtx.GetNumRows() - 1) * rowExtent, rowExtent, 0, NULL, NULL);
	...
	clErr = clEnqueueNDRangeKernel(queue, copyRectKernel,
				       1,		  // work dimensions
	...
	clErr = clWaitForEvents(waitEvents.size(), waitEvents.empty() ? NULL : &waitEvents.front());
	CL_CHECK_ERROR(clErr);
	ClearWaitEvents(waitEvents);

	// do the stencil iterations
	for (int iter = 0; iter < nIters; iter++) {
		this->DoPreIterationWork(*currData, *newData, mtx, iter, queue);
		...
		SetStencilKernelArgs(*currData, *newData, (cl_int)(mtx.GetPad()), this->wCenter,
				     this->wCardinal, this->wDiagonal, localDataSize);
		...
		lWorkDims[0] = 1;
		lWorkDims[1] = lCols;
		clErr = clEnqueueNDRangeKernel(queue, kernel,
		...
		if (iter == nIters - 1) {
			...
			std::cout << "[DEBUG] kernel runtime:: " << elapsed / 1000000.0f
				  << std::endl;
			// std::cout << "begin: " << time_start << " end: " << time_end <<
			// std::endl;
...
// wait for this iteration to complete.
			...
			clErr = clGetEventInfo(mainKernelEvt, CL_EVENT_COMMAND_QUEUE,
					       sizeof(clqueue), &clqueue, NULL);
			CL_CHECK_ERROR(clErr);
			std::cerr << "queue=" << queue << ", clqueue=" << clqueue << std::endl;

			cl_context clctxt;
			clErr = clGetEventInfo(mainKernelEvt, CL_EVENT_CONTEXT, sizeof(clctxt),
					       &clctxt, NULL);
			CL_CHECK_ERROR(clErr);
			std::cerr << "ctx=" << context << ", clctxt=" << clctxt << std::endl;
			cl_command_type clct;
			clErr = clGetEventInfo(mainKernelEvt, CL_EVENT_COMMAND_TYPE, sizeof(clct),
					       &clct, NULL);
			CL_CHECK_ERROR(clErr);
			std::cerr << "command type=" << clct << std::endl;
			cl_int cles;
			clErr = clGetEventInfo(mainKernelEvt, CL_EVENT_COMMAND_EXECUTION_STATUS,
					       sizeof(cles), &cles, NULL);
			CL_CHECK_ERROR(clErr);
...
template <class T>
void OpenCLStencil<T>::DoPreIterationWork(cl_mem buf, cl_mem altBuf, Matrix2D<T> &mtx,
					  unsigned int iter, cl_command_queue queue) {
